...
export function WorkshopTodoPage() {
  ...
  function handleDrop(event: DragEvent, status: WorkshopTaskStatus) {
    ...
    if (draggedTaskId) moveTask(organization.id, draggedTaskId, status)
  ...
  return (
    <div className="workshop-page workshop-board-page">
      <section className="workshop-page-heading compact">
        ...
        <Button className="workshop-page-action" onClick={() => setAddingTo("pending")} type="button"><Plus /> Add task</Button>
      </section>
      ...
      <div className="workshop-board">
        {columns.map(({ description, icon: Icon, id, label }) => {
          ...
          return (
            <section className="workshop-board-column" key={id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => handleDrop(event, id)}>
              <header><span><Icon /><strong>{label}</strong><b>{columnTasks.length}</b></span><p>{description}</p></header>
              <div className="workshop-task-list">
                {columnTasks.map((task) => <TaskCard key={task.id} organization={organization} onDragStart={() => setDraggedTaskId(task.id)} task={task} />)}
                <Button className="workshop-add-task" onClick={() => setAddingTo(id)} type="button" variant="ghost"><Plus /> Add a task</Button>
              </div>
            </section>
          )
        })}
...
function TaskCard({
  ...
  return (
    <Card className="workshop-task" draggable onDragStart={onDragStart}>
      <header><span className={`priority-${task.priority.toLowerCase()}`}>{task.priority}</span><Button aria-label="Task actions" size="icon-xs" type="button" variant="ghost"><MoreHorizontal /></Button></header>
      <div><GripVertical /><h3>{task.title}</h3></div>
      {task.description && <p>{task.description}</p>}
      <footer><span>{task.label}</span>{assignee ? <b title={assignee.name}>{assignee.initials}</b> : <Button aria-label="Assign task" size="icon-xs" type="button" variant="ghost"><Plus /></Button>}</footer>
    </Card>
  )
}
